...
// copied from http://schemas.microsoft.com/SQL/Notifications/QueryNotification/QueryNotification.xsd
...
function watch(db, args) {
    ...
    function watchImpl(cb) {
        ...
        checkSubscriptionImpl(blocking, function (err, result) {
            ...
            // some other modification of the table happened (e.g. table altered, new column added)
            // TODO: is simply re-subscribing the best option here?
            if (VALID_SOURCE_TYPES[result.source] && VALID_INFO_TYPES[result.info]) {
                return subscribeAll(cb);
            }
